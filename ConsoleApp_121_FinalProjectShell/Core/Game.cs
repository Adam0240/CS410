...
public partial class Game
{
    ...
    internal void CreateRooms()
    {
        ...
        hub = new Room($"This campsite, used by travelers passing through, right now houses only you. \nA fitting {TextColors.CYAN}place to rest{TextColors.NORMAL} when the job is done.", 0);
        swamp = new Room($"Your boots catch in the stiff and stinking muck of the swamp. \nA large {TextColors.CYAN}fallen log{TextColors.NORMAL} sits to one side.", 1);
        battleGr = new Room("The site of a once great battle stands silent, unusable weapons dotting the landscape.", 2);
        rocky = new Room($"Standing at the remains of a collapsed quarry, you can see {TextColors.CYAN}veins of ore{TextColors.NORMAL} within the stone.", 3);
        lava = new Room($"Lava flows through channels dug into the rock around a vacant smith's shop, \nthe {TextColors.CYAN}tools{TextColors.NORMAL} of which seem strangely lacking.", 4);
        ...
        axe = ItemFactory.Create("axe", $"a battered war {TextColors.YELLOW}AXE{TextColors.NORMAL}", 55, 0);
        ring = ItemFactory.Create("ring", $"a shining {TextColors.YELLOW}RING{TextColors.NORMAL} with a knight's insignia", 2, 1);
        hammer = ItemFactory.Create("hammer", $"a standard issue craft {TextColors.YELLOW}HAMMER{TextColors.NORMAL} with a flat head", 34, 2);
        ore = ItemFactory.Create("ore", $"a chunk of unrefined {TextColors.YELLOW}ORE{TextColors.NORMAL}", 46, 3);
        hilt = ItemFactory.Create("hilt", $"a {TextColors.YELLOW}HILT{TextColors.NORMAL} of an old sword", 17, 4);
        sword = ItemFactory.Create("sword", $"a sharp {TextColors.YELLOW}SWORD{TextColors.NORMAL} with a regal gleam", 22, 5);
        ...
        oldHorseFollower.AddIdleText($"Your {TextColors.BLUE}mule{TextColors.NORMAL} brays softly.");
    }
    ...
    {
        Console.WriteLine();
        Console.WriteLine($"Welcome to {TextColors.CYAN}A Clueless Traveler{TextColors.NORMAL}!");
        Console.WriteLine("Help the dumb protagonist have the slimmest chance to survive.");
        Console.WriteLine($"Type '{TextColors.GREEN}help{TextColors.NORMAL}' if you need help.");
        Console.WriteLine();
...
if (room != null)
{
    ...
    internal void PrintHelp(Command command)
    {
        if (!command.HasSecondWord())
        {
            ...
            Console.WriteLine("- Secure a weapon for the protagonist.");
            Console.WriteLine("- Open a path through the castle gate.");
            Console.WriteLine($"Tip: use '{TextColors.GREEN}talk{TextColors.NORMAL}' near the protagonist for dynamic objective hints.");
            Console.WriteLine();
        ...
        {
            case CommandWord.GO:
                Console.WriteLine($"The '{TextColors.GREEN}go{TextColors.NORMAL}' command moves you between rooms." +
                                  "\nSynonyms: walk, move" +
                                  "\nUsage: 'go north'");
                break;
            ...
            case CommandWord.QUIT:
                Console.WriteLine($"The '{TextColors.GREEN}quit{TextColors.NORMAL}' command exits the game. Progress is not auto-saved.");
                break;
            case CommandWord.BACK:
                Console.WriteLine($"The '{TextColors.GREEN}back{TextColors.NORMAL}' command returns you to the most recently visited room.");
                break;
            case CommandWord.LOOK:
                Console.WriteLine($"The '{TextColors.GREEN}look{TextColors.NORMAL}' command displays the current room description.");
                break;
            case CommandWord.TAKE:
                Console.WriteLine($"The '{TextColors.GREEN}take{TextColors.NORMAL}' command picks up an item in the room." +
                                  "\nUsage: 'take hammer'");
                break;
            case CommandWord.DROP:
                Console.WriteLine($"The '{TextColors.GREEN}drop{TextColors.NORMAL}' command leaves an inventory item in the room." +
                                  "\nUsage: 'drop hammer'");
                break;
            case CommandWord.ITEMS:
                Console.WriteLine($"The '{TextColors.GREEN}items{TextColors.NORMAL}' command lists your inventory and carry weight.");
                break;
            case CommandWord.USE:
                Console.WriteLine($"The '{TextColors.GREEN}use{TextColors.NORMAL}' command uses an item from your inventory." +
                                  "\nUsage: 'use axe'");
                break;
            case CommandWord.TALK:
                Console.WriteLine($"The '{TextColors.GREEN}talk{TextColors.NORMAL}' command speaks to the protagonist (must be in the same room)." +
                                  "\nIt gives dynamic objective guidance and acknowledges completed tasks.");
                break;
            case CommandWord.SLEEP:
                Console.WriteLine($"The '{TextColors.GREEN}sleep{TextColors.NORMAL}' command ends the run only when both objectives have been delivered to the protagonist." +
                                  "\nYou must be at camp to sleep.");
                break;
            case CommandWord.FOLLOW:
                Console.WriteLine($"The '{TextColors.GREEN}follow{TextColors.NORMAL}' command unties your {TextColors.BLUE}mule{TextColors.NORMAL} so it follows you.");
                break;
            case CommandWord.STAY:
                Console.WriteLine($"The '{TextColors.GREEN}stay{TextColors.NORMAL}' command tethers your {TextColors.BLUE}mule{TextColors.NORMAL} to keep it in place.");
                break;
            case CommandWord.TRADE:
                Console.WriteLine($"The '{TextColors.GREEN}trade{TextColors.NORMAL}' command exchanges items with your mule.");
                break;
            case CommandWord.SAVE:
                Console.WriteLine($"The '{TextColors.GREEN}save{TextColors.NORMAL}' command writes current progress to disk.");
                break;
            case CommandWord.LOAD:
                Console.WriteLine($"The '{TextColors.GREEN}load{TextColors.NORMAL}' command restores progress from disk.");
                break;
            case CommandWord.DELETE:
                Console.WriteLine($"The '{TextColors.GREEN}delete{TextColors.NORMAL}' command erases existing save data.");
                break;
            default:
    ...
    internal void Take(Command command)
    {
        ...
        if (tempItem != null)
        {
            if (activePlayer.isValidItem(tempItem))
            {
                ...
                Console.WriteLine("Picked up the " + TextColors.YELLOW + tempItem.GetName() + TextColors.NORMAL + "!");
                ...
                if (tempItem.GetName() == "hammer" && activePlayer.GetCurrentRoom()!.GetId() == 4)
                {
                    ...
                    Console.WriteLine($"The forge's {TextColors.CYAN}tool set{TextColors.NORMAL} is once again incomplete.");
                }
            }
            else
            {
    ...
    internal void Drop(Command command)
    {
        ...
        if (tempItem != null)
        {
            ...
            Console.WriteLine("Dropped the " + TextColors.YELLOW + tempItem.GetName() + TextColors.NORMAL + "!");
        }
        else
        {
    ...
    internal void Follow(Command _)
    {
        ...
        {
            Console.WriteLine($"Your trusty {TextColors.BLUE}mule{TextColors.NORMAL} isn't around right now.");
            return;
        }

        if (oldHorseFollower.IsFollowing())
        {
            Console.WriteLine($"Your {TextColors.BLUE}mule{TextColors.NORMAL} is already following you!");
            return;
        }
        Console.WriteLine($"You untether your {TextColors.BLUE}mule{TextColors.NORMAL}.");
    ...
    internal void Stay(Command _)
    {
        ...
        if (!oldHorseFollower.IsFollowing())
        {
            Console.WriteLine($"Your {TextColors.BLUE}mule{TextColors.NORMAL} has already been tethered here!");
            return;
        }
        Console.WriteLine($"You tie your {TextColors.BLUE}mule's{TextColors.NORMAL} reins to a nearby post or fixture.");
    ...
    private bool TryTrade(string word)
    {
        ...
        {
            Console.WriteLine("Gave the " + TextColors.YELLOW + word + TextColors.NORMAL + $" to your {TextColors.BLUE}mule{TextColors.NORMAL}.");
            return false;
        }

        if (oldHorseFollower.GiveToPlayer(activePlayer, word))
        {
            Console.WriteLine("Took the " + TextColors.YELLOW + word + TextColors.NORMAL + $" from your {TextColors.BLUE}mule{TextColors.NORMAL}.");
            return false; 
        }
    ...
    {
        ItemsPrint();
        Console.WriteLine($"[Type {TextColors.GREEN}QUIT{TextColors.NORMAL} to stop trading]");
    }
    ...
    private List<string> BuildProtagonistReminders()
    {
        ...
        {
            reminders.Add($"\"The buildings and terrain around here are unstable, {TextColors.CYAN}something with heft{TextColors.NORMAL} could be handy for navigation.\"");
        }
        ...
        if (needsGatePath)
        {
            reminders.Add($"\"That {TextColors.CYAN}castle gate{TextColors.NORMAL} is shut fast. Its sturdy, but still only made of wood...\"");
        }
    ...
    private string BuildSwordObjectiveHint()
    {
        ...
        if (hasSword)
        {
            return $"\"I'll have to search that {TextColors.CYAN}grove{TextColors.NORMAL} for a weapon again later.\"";
        }

        // Axe is only relevant for sword path before swamp is cleared.
        if (!progress.SwampCleared)
        { 
            return $"\"I've heard stories about a shrine in that {TextColors.CYAN}swamp{TextColors.NORMAL}, but I saw no path to follow. " +
                   "\nCould the way be blocked by something?\"";
        }
        if (!hasHilt)
        {
            return $"\"I had thought that {TextColors.CYAN}grove{TextColors.NORMAL} might have a {TextColors.CYAN}weapon{TextColors.NORMAL} suited for me, but unfortunately not. " +
                   "\nPerhaps I'll have to check again later.\"";
        }

        if (!hasHammer || !hasOre)
        {
            return $"\"They say the smiths of this land produced many great weapons, the blows of their {TextColors.CYAN}hammers{TextColors.NORMAL} " +
                   $"\nrefining their {TextColors.CYAN}metal{TextColors.NORMAL} into gleaming blades.\"";
        }
        
        return $"\nI've heard that that the {TextColors.CYAN}ore{TextColors.NORMAL} of this land requires an extremely {TextColors.CYAN}powerful{TextColors.NORMAL} heat to be properly forged.\n";
    }
    ...
    {
        Console.WriteLine($"In a single mighty blow, you {TextColors.RED}strike down{TextColors.NORMAL} the oblivious protagonist.");
